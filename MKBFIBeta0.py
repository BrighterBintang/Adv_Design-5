'''
Welcome to MKBFIBeta0, short for Merged Kostas Beer Fermentation Isothermal: Beta Edition, Version 0
Compared to MKBFIAlpha1, this Python file:
- Uses Josh's Most High-End Code: HBFAlpha (except mass balance)
- Adds the shaded region of the EDS (both parameters) in both the graphs (ethanol yield and sugar consumption rates)

In terms of faults:
- There is no shaded region of the EDS (both parameters) in the sugar consumption rates graphs, probably caused by not properly recalculating the rates.
This issue needs to be fixed to qualify for MKBFIBeta1 and beyond.
- The One-At-a-Time Response Curves do not have units, because the code bundles the graphs together.
So, individual y-axes cannot be made in this format. (If fixed, they will be in a future, near-final edition)
'''
...
# maximum specific growth rates in h^-1 # updated for the first run
V_G = V_G0 * np.exp(-E_VG / (1.987 * T0_base))  # R = 1.987 cal/mol*K
V_M = V_M0 * np.exp(-E_VM / (1.987 * T0_base))
V_N = V_N0 * np.exp(-E_VN / (1.987 * T0_base))
...
K_G = np.exp(-121.3) * np.exp(-(-68600) / (1.987 * T0_base))
K_M = np.exp(-19.15) * np.exp(-(-14400) / (1.987 * T0_base))
K_N = np.exp(-26.78) * np.exp(-(-19900) / (1.987 * T0_base))
...
K_G_prime = np.exp(23.33) * np.exp(-10220 / (1.987 * T0_base))
K_M_prime = np.exp(55.61) * np.exp(-26350 / (1.987 * T0_base))
...
Y_EAS = 1.237e-3 + (T - 14.5) * ((1.237e-3 - 9.870e-4) / (
            14.5 - 12))  # molar ratio - https://onlinelibrary.wiley.com/doi/epdf/10.1002/j.2050-0416.1994.tb00830.x
...
# Low temperature
set_temp_kinetics(T - 2)
t, solution_T_low = solve_odes(G0, M0, N0, X0, E0, CO20, VDK0, EA0)

ethanol_T_low = solution_T_low[:, 4]
mu1_T_low, mu2_T_low, mu3_T_low = compute_sugar_rates(solution_T_low)

# High temperature
set_temp_kinetics(T + 2)
t, solution_T_high = solve_odes(G0, M0, N0, X0, E0, CO20, VDK0, EA0)

ethanol_T_high = solution_T_high[:, 4]
mu1_T_high, mu2_T_high, mu3_T_high = compute_sugar_rates(solution_T_high)
...
ethanol_yield_T_high = np.divide(ethanol_T_high, sugar_consumed, out=np.zeros_like(ethanol_T_high), where=sugar_consumed!=0)
ethanol_yield_T_low = np.divide(ethanol_T_low, sugar_consumed, out=np.zeros_like(ethanol_T_low), where=sugar_consumed!=0)
...
ax1.fill_between(time, ethanol_yield_T_high, ethanol_yield_T_low, alpha=0.2, label='Temperature ±2 K')
...
# Sugar Consumption Rates (with Initial Sugar Mass Deviation)
ax2.plot(time, mu1, 'b-', label='Glucose Rate (μ₁)')
ax2.fill_between(time, mu1_s_high, mu1_s_low, color = (0, 0, 1, 0.2), alpha=0.2, label='Sugar ±5%')
ax2.plot(time, mu2, 'g-', label='Maltose Rate (μ₂)')
ax2.fill_between(time, mu2_s_high, mu2_s_low, color = (0, 1, 0, 0.2), alpha=0.2, label='Sugar ±5%')
ax2.plot(time, mu3, 'r-', label='Maltotriose Rate (μ₃)')
ax2.fill_between(time, mu3_s_high, mu3_s_low, color = (1, 0, 0, 0.2), alpha=0.2, label='Sugar ±5%')
...
# Sugar Consumption Rates (with Initial Temperature Deviation)
ax3.plot(time, mu1, 'b-', label='Glucose Rate (μ₁)')
ax3.fill_between(time, mu1_T_high, mu1_T_low, color = (0, 0, 1, 0.2), alpha=0.2, label='Temperature ±2 K')
ax3.plot(time, mu2, 'g-', label='Maltose Rate (μ₂)')
ax3.fill_between(time, mu2_T_high, mu2_T_low, color = (0, 1, 0, 0.2), alpha=0.2 , label='Temperature ±2 K')
ax3.plot(time, mu3, 'r-', label='Maltotriose Rate (μ₃)')
ax3.fill_between(time, mu3_T_high, mu3_T_low, color = (1, 0, 0, 0.2), alpha=0.2, label='Temperature ±2 K')
ax3.set_title('Specific Sugar Consumption Rates')
ax3.set_ylabel('Specific Rate (h⁻¹)')
...
# =============
# Mass Balance (excluded in this edition, but can be re-included)
# =============
...
set_temp_kinetics(T0_base)
baseline = run_fermentation(G0_base, M0_base, N0_base, T0_base)

print(f"baseline Ethanol: {baseline['final_ethanol']:.3f} mol/m³")
print(f"baseline Fermentation Time: {baseline['fermentation_time']:.3f} h")
...
# Sugar mass range: ±5%
sugar_factors = (0.95, 1.00, 1.05)
sugar_results = {key: [] for key in baseline.keys()}

for f in sugar_factors:
    res = run_fermentation(G0_base * f, M0_base * f, N0_base * f, T0_base)
    for key in res:
        sugar_results[key].append(res[key])

    if f != 1.0:
        diff_ethanol = res['final_ethanol'] - baseline['final_ethanol']
        pct_change = (diff_ethanol / baseline['final_ethanol']) * 100

# Temperature range: ±2 °C (isothermal → flat curves expected)
temperature_values = (T0_base - 2, T0_base, T0_base + 2)
temp_results = {key: [] for key in baseline.keys()}

for T_val in temperature_values:
    set_temp_kinetics(T_val)

    res = run_fermentation(G0_base, M0_base, N0_base, T0_base)

    for key in res:
        temp_results[key].append(res[key])

    if T_val != T0_base:
        diff_time = res['fermentation_time'] - baseline['fermentation_time']
...
"""
SUBCHAPTER 3.2) Print of Normalized Sensitivity
"""
...
for output_key, base_val in baseline.items():
    ...
    val_plus = temperature_perturb(1.1)[output_key]
    val_minus = temperature_perturb(0.9)[output_key]

    # temperature sensitivity (expected ~0)
    S_temp = (val_plus - val_minus) / (2 * delta_T)
