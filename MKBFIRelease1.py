'''
Welcome to MKBFIRelease1, short for Merged Kostas Beer Fermentation Isothermal: Release Edition, Version 1
Compared to MKBFIBeta1, this Python file:
- Uses Kostas's Final Code: KBFRelease (except mass balance)
- Adds the shaded region of the EDS (both parameters) in both the graphs (ethanol yield and sugar consumption rates)
- Once-At-a-Time Graphs and Code are scrapped (no longer relevant)

In terms of faults:
- Although the code for the temperature shaded areas is present, those areas are not showing up in the graphs. 
Either initial temperature has no effect, or Josh's "set_temp_kinetics" code is required and the shaded areas may appear.
If the temperature shaded areas are real, then that would be present in the next edition.
'''
...
# ax1.fill_between(time, G_Tp, G_Tm, alpha=0.2, label='Temperature ±2 K')
...
# ax1.fill_between(time, M_Tp, M_Tm, alpha=0.2, label='Temperature ±2 K')
...
# ax1.fill_between(time, N_Tp, N_Tm, alpha=0.2, label='Temperature ±2 K')
...
# ax2.fill_between(time, bmc_Tp, bmc_Tm, alpha=0.2, color= (0, 0, 0, 0.2), label='Temperature ±2 K')
...
# ax3.fill_between(time, E_Tp, E_Tm, alpha=0.2, color= (1, 0.5, 0, 0.2), label='Temperature ±2 K')
