'''
Welcome to MKBFIAlpha1_lite, short for Merged Kostas Beer Fermentation Isothermal: Alpha Edition, Version 1 lite
Compared to MKBFI3, this Python file:
- Uses Kostas's Most High-End Code: KBFAlpha (except mass balance)
- Tweaks the Solve ODEs code to run the external disturbance scenario (EDS) of ±5% initial sugar mass
- Adds the shaded region of the EDS in the ethanol yield graph
- The One-At-a-Time Response Curves are added back

In terms of faults:
- Line 403: "RuntimeWarning: invalid value encountered in divide", because did not use np.divide for now
- The One-At-a-Time Response Curves do not have units, because the code bundles the graphs together. So, individual y-axes cannot be made in this format

Just publishing this pre-Version to make it visible for the team and take accountability for my progress
'''
...
ax1.fill_between(time, ethanol_s_high / sugar_consumed, ethanol_s_low / sugar_consumed, alpha=0.2, label='Sugar ±5%')
...
# Sugar mass range: ±5%
sugar_factors = np.linspace(0.95, 1.05, 15)

# Temperature range: ±2 °C (isothermal → flat curves expected)
temperature_values = np.linspace(T0_base - 2, T0_base + 2, 15)
